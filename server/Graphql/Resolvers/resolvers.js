...
const resolvers = {
  ...
  Mutation: {
    ...
    async createEvent(_, args, context) {
      ...
      const errors = {};
      const eventPresent = await Event.findOne({ eventBelongsTo, eventType });
      ...
      if (
        ...
        throw new UserInputError("Multiple same events", { errors });
      } else {
    ...
    async updateEvent(_, args, context) {
      ...
      if (eventPresent) {
        if (eventPresent.userId === userId) {
          ...
          throw new AuthenticationError("Change not Authorized", { errors });
        }
      } else {
        errors.general = "The event does not exist";
        throw new UserInputError("Event does not exist", { errors });
      }
    },
  },
};
